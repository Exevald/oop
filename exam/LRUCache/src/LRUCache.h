...
{
public:
	explicit LRUCache(size_t capacity) noexcept;
	~LRUCache() = default;
...
template <typename Key, typename Value>
LRUCache<Key, Value>::LRUCache(size_t capacity) noexcept
	: m_capacity(capacity)
{
	assert(m_capacity > 0);
}
...
template <typename Key, typename Value>
LRUCache<Key, Value>::LRUCache(const LRUCache& other)
...
template <typename Key, typename Value>
LRUCache<Key, Value>& LRUCache<Key, Value>::operator=(const LRUCache& other)
{
	if (this == &other)
	{
		return *this;
...
template <typename Key, typename Value>
Value* LRUCache<Key, Value>::Get(const Key& key) noexcept
{
...
template <typename Key, typename Value>
Value* LRUCache<Key, Value>::GetValueOrDefault(const Key& key, std::function<Value()> factory)
{
	auto it = m_itemsMap.find(key);
	if (it != m_itemsMap.end())
	{
...
template <typename Key, typename Value>
void LRUCache<Key, Value>::RemoveOverflowIfExists() noexcept
{
